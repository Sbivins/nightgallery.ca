...
<div class="wid">
	...
	<div class="contents" id="upcoming">
		...
		<?php
			...
			$query .= "WHERE event_start >= CURRENT_DATE() AND event_type < 4 ORDER BY event_start ASC";
			...
			while( $content = mysql_fetch_array($allevents2) ){
				...
				//the following runs each time:
					...
					echo "<a class=\"eventinfo\" href=\"/event.php?id=" . $content['event_id'] . "&name=" . add_dashes($content['event_title']) . "\">";
			...
			if(!$allevents){
				...
				echo "<div class=\"timeframe\">NEXT SHOW</div>";
					...
					echo "<a class=\"eventinfo\" href=\"/event.php?id=" . $id . "&name=" . add_dashes($row['event_title']) . "\">";
					echo "<h3 class=\"eventtitle\">";
