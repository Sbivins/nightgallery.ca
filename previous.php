...
<?php build_head("NIGHT GALLERY | PREVIOUS EXHIBITIONS"); ?>
...
<div class="wid">
	...
	<div class="header">
		...
		<?php build_nav("previous") ?>
		
	</div>


	<div class="contents" id="previouspage">
		...
		<div id="previouslx">
			...
			<?php
				...
				$query .= "WHERE event_end <= CURRENT_DATE() AND event_type<>4 ORDER BY event_start DESC";
				...
				testquery($contentset);
					...
					while($content = mysql_fetch_array($contentset)){
							...
							if (get_year($content['event_start']) != $year) {
								...
								echo "<li class=\"eventyear " . $year . "set\">";
								echo $year;
								echo "</li>";
								
								if ($counter>1){
									echo "<span class=\" " . $year . "set\">";
								}
							}
							
							echo "<li class=\"eventunit\">";
							echo "<a href=\"/event.php?id=" . $content['event_id'] . "\" class=\"event" . $content['event_id'] . " calitem inactive\">";
								echo "<p class=\"title\">";
		...
		while($content2 = mysql_fetch_array($contentset2)){
			...
			//echo "<p>ARTIST3</p>
			echo "</div></div>";
			echo "<div class=\"preview-poster preloadcontain\" style=\"background-image:url(" . $content2['event_pic_url'] . ")\">&nbsp;</div>";
